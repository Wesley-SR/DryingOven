...
// CWT-TH04S registers: humidity first, then temperature
...
SensorTH04S::SensorTH04S(SoftwareSerial& serial, uint8_t slaveAddress)
    : _serial(serial),
...
SensorTH04SError SensorTH04S::read()
{
    ...
    // Remove stale bytes before starting a new Modbus request
...
bool SensorTH04S::sendReadRequest()
{
    ...
    // The RS485 converter handles TX/RX direction automatically
    size_t bytesWritten = _serial.write(request, sizeof(request));
    _serial.flush();
...
{
    uint16_t receivedCRC = response[length - 2] | (static_cast<uint16_t>(response[length - 1]) << 8);
...
{
    uint16_t humidityRaw = (static_cast<uint16_t>(response[3]) << 8) | response[4];
    int16_t temperatureRaw = static_cast<int16_t>((static_cast<uint16_t>(response[5]) << 8) | response[6]);

    _humidity = humidityRaw / 10.0f;
    _temperature = temperatureRaw / 10.0f;
}
