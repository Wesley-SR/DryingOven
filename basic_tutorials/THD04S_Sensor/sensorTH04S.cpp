...
// Verify these register addresses in the CWT-TH04S datasheet.
#define TH04S_START_REGISTER 0x0000
#define TH04S_REGISTER_COUNT 2

// Response:
// Address + Function + Byte Count + Humidity + Temperature +  + CRC
...
void SensorTH04S::processResponse(const uint8_t* response) {
    uint16_t humidityRaw = (response[3] << 8) | response[4];
    int16_t temperatureRaw = (int16_t)((response[5] << 8) | response[6]);
